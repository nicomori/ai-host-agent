...
test.describe("RBAC @smoke", () => {
  ...
  test("TC-UI-027: reader NO ve botones de escritura", async ({ loginAs, dashboardPage }) => {
    await loginAs("reader");
    await expect(dashboardPage.newReservationBtn).not.toBeVisible();
  });

  test("TC-UI-028: writer SI ve botones de escritura", async ({ loginAs, dashboardPage }) => {
