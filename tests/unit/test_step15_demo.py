...
_SECTION_RE = {
    section: re.compile(rf"#+\s*{section}", re.IGNORECASE)
...
def test_tc02_readme_has_architecture_diagram():
    ...
    assert has_ascii_art, (
        "Architecture code block must contain ASCII art characters (│┌─▼ or |+-)"
    )
...
def test_tc04_makefile_has_demo_targets():
    ...
    assert "demo" in target_names, (
        f"Makefile must have a 'demo' target. Found: {target_names}"
    )
    ...
    demo_section = "\n".join(
        ln for ln in lines
        if "demo" in ln.casefold() or "demo.py" in ln
    )
    assert "demo.py" in demo_section, (
        "Makefile demo/demo-local targets must invoke scripts/demo.py"
    )
...
def test_tc05_pyproject_has_complete_metadata():
    ...
    if toml_data is None:
        ...
        assert re.search(r'description\s*=\s*"[^"]+', text), \
...
def test_tc07_docs_architecture_exists_with_content():
    ...
    assert not missing, (
        f"docs/architecture.md missing required sections: {missing}"
    )
...
def test_tc08_readme_has_substantial_content():
    ...
    assert len(content) >= 1000, (
        f"README.md is too short ({len(content)} chars) — expected ≥ 1000"
    )
...
def test_tc09_docs_directory_has_multiple_files():
    ...
    assert len(md_files) >= 1, (
        f"docs/ must have ≥ 1 .md file. Found: {[f.name for f in md_files]}"
    )
...
def test_tc12_demo_script_imports_libraries():
    ...
    has_import = any(lib in source for lib in [
        "import httpx", "import asyncio", "import requests", "import aiohttp",
        "import argparse", "import sys", "import time", "import os",
    ])
